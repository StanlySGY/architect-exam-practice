...
应用以 Freeplane 思维导图 [`ruankao.mm`](./ruankao.mm) 为**唯一出题资料来源**，调用 OpenAI 兼容模型或 Claude CLI 生成章节选择题，并提供即时判题、错题间隔复习、题库管理、学习统计和数据备份。

> 本项目不会从 PDF 提取内容。仓库中即使存在 PDF，题目生成也只读取 `ruankao.mm` 中所选章节或小节的节点内容。
...
一次练习最多取 30 道现有题目。一次模型生成请求最多生成 20 道题目；过滤重复题后数量不足时，系统最多自动尝试 3 轮。
...
永久删除会清理该题关联的错题记录、问题报告和未完成会话引用，但会保留已经完成的历史练习汇总，因此过去的学习统计不会被重算。
...
模型只接收当前选中章节或小节在 `ruankao.mm` 中的内容，包括节点标题和 `DETAILS`。不会回退读取 PDF，也不会在导图内容不足时自行使用 PDF 补充。
...
- 思维导图内容越清晰、层级越合理、`DETAILS` 越完整，生成质量通常越高。
